...
void OV2640::rotateImage180(const uint8_t *src, int width, int height, uint8_t *dest)
{
    if (!src || width <= 0 || height <= 0 || !dest)
    {
        ESP_LOGE(TAG, "Invalid parameters for image rotation");
        return;
    }
    ...
    for (int y = 0; y < height; y++)
    {
        ...
        {
            int srcIndex = (y * width + x) * 3; // Assuming 3 bytes per pixel (RGB)
            int destIndex = ((height - y - 1) * width + (width - x - 1)) * 3;
